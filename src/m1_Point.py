...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distancetravelled = self.distancetravelled + ((y - self.y)**2 + (x - self.x)**2)**0.5
        self.x = x
        self.y = y
    ...
    def move_by(self, dx, dy):
        ...
        self.y = self.y + dy
        self.moves = self.moves + 1
        self.distancetravelled = self.distancetravelled + ((dx**2) + (dy**2))**0.5
    ...
    def halfway_to(self, p2):
        midpoint = Point(((self.x + p2.x)/2),((self.y +p2.y)/2))
        return midpoint
